      ...
      verifyIdToken: async (idToken, checkRevoked = false) => {
        ...
        if (idToken && idToken.includes(".") && idToken.split(".").length === 3) {
          try {
            ...
            console.log("✅ Mock decoded JWT:", {
              alg: header.alg,
              email: payload.email || payload.user_email || "unknown@example.com",
              uid: payload.user_id || payload.sub || `mock-uid-${Date.now()}`,
            });
  ...
  try {
    ...
    if (firebaseInitialized && admin.apps.length > 0) {
      ...
      try {
        ...
        console.log(`   UID: ${decodedToken.uid}, Verified: ${decodedToken.email_verified}`);
      } catch (firebaseError) {
        console.log(
          `❌ Firebase Admin verification failed: ${firebaseError.code} - ${firebaseError.message}`
        );
    ...
    if (!decodedToken && (authMethod === "jwt_decode" || authMethod === "mock_no_firebase" || authMethod === "mock_fallback")) {
      try {
        // Check if it looks like a JWT
        if (idToken.includes(".") && idToken.split(".").length === 3) {
          const parts = idToken.split(".");
          ...
          try {
            ...
            console.log(`✅ JWT decoded successfully: ${payload.email || payload.user_email || "no email"}`);
    ...
    if (dbConnected) {
      try {
        ...
        if (!user) {
          ...
          let role = "user";
          if (decodedToken.email === "mahdiashan9@gmail.com" || decodedToken.email === "islameshan451@gmail.com") {
            role = "admin";
            console.log(`⭐ Setting ${decodedToken.email} as admin`);
          }
          ...
          if ((user.email === "mahdiashan9@gmail.com" || user.email === "islameshan451@gmail.com") && user.role !== "admin") {
            console.log(`⭐ Upgrading ${user.email} to admin role`);
            user.role = "admin";
          }
        ...
        console.error("❌ Database error during user lookup/creation:", dbError.message);
        ...
        user = {
          ...
          name: decodedToken.name || decodedToken.email.split("@")[0],
          photoURL: decodedToken.picture,
          role: decodedToken.email === "mahdiashan9@gmail.com" || decodedToken.email === "islameshan451@gmail.com" ? "admin" : "user",
          emailVerified: decodedToken.email_verified || true,
          lastLogin: new Date(),
        };
      }
...
const requireRole = (roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.mongoUser.role)) {
      console.log(`🔒 Access denied for ${req.mongoUser.email}: Required role: ${roles.join(", ")}, Current role: ${req.mongoUser.role}`);
      return res.status(403).json({
        success: false,
...
app.get("/api/my-bookings", firebaseAuthMiddleware, async (req, res) => {
  try {
    console.log(`📋 Getting bookings for user (compatibility endpoint): ${req.mongoUser.email}`);
...
// ========== FIXED: VENDOR ROUTES ==========
// Vendor dashboard stats
app.get("/api/vendor/dashboard/stats", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    console.log(`📊 Vendor dashboard stats requested by: ${req.mongoUser.email}`);
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      // Count pending applications for vendor's cards
      const vendorCards = await Ticket.find({ vendorId: vendorId }).select("_id");
      const cardIds = vendorCards.map((card) => card._id);
...
// Get vendor's cards/tickets
app.get("/api/vendor/cards", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    console.log(`🎟️ Vendor cards requested by: ${req.mongoUser.email}`);
    ...
    if (mongoose.connection.readyState === 1) {
      const vendorId = req.mongoUser.uid;
      cards = await Ticket.find({ vendorId: vendorId }).sort({ createdAt: -1 });
    } else {
      // Mock data
      cards = [
        {
...
app.get("/api/vendor/cards/:id", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
...
// Create new card/ticket (vendor)
app.post("/api/vendor/cards", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    console.log(`➕ Creating new card for vendor: ${req.mongoUser.email}`);
...
// Update vendor card
app.put("/api/vendor/cards/:id", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
...
// Delete vendor card
app.delete("/api/vendor/cards/:id", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`🗑️ Deleting vendor card ${id}`);
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      if (card.vendorId !== req.mongoUser.uid && req.mongoUser.role !== "admin") {
        return res.status(403).json({
          success: false,
          message: "You don't have permission to delete this card",
        });
...
// Get vendor's applications (bookings for vendor's cards)
app.get("/api/vendor/applications", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { status = "all" } = req.query;
    console.log(`📋 Vendor applications requested by: ${req.mongoUser.email}, status: ${status}`);
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      // Get vendor's cards
      const vendorCards = await Ticket.find({ vendorId: vendorId }).select("_id title price");
      const cardIds = vendorCards.map((card) => card._id);
...
// Approve/Reject application
app.put("/api/vendor/applications/:id/:action", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id, action } = req.params;
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      if (action === "approve" && updatedApplication.ticketId) {
        const ticket = await Ticket.findById(updatedApplication.ticketId);
        if (ticket) {
          ticket.availableQuantity = Math.max(0, ticket.availableQuantity - updatedApplication.quantity);
          await ticket.save();
          console.log(`✅ Reduced available quantity for ticket ${ticket._id}`);
        }
      }
    } else {
      // Mock response
...
app.get("/api/vendor/profile", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
...
app.put("/api/vendor/profile", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
...
app.get("/api/vendor/bookings", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { status = "all" } = req.query;
...
app.put("/api/vendor/bookings/:id/accept", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
...
app.put("/api/vendor/bookings/:id/reject", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
...
app.put("/api/vendor/bookings/:bookingId/status", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    const { bookingId } = req.params;
    const { status } = req.body;

    console.log(`📝 Updating vendor booking ${bookingId} status to: ${status}`);
...
app.get("/api/vendor/revenue", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      // Calculate total revenue
      revenueData.totalRevenue = allBookings.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
      ...
      revenueData.pending = pendingBookings.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
      ...
      revenueData.thisMonth = thisMonthBookings.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
      ...
      revenueData.lastMonth = lastMonthBookings.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0);
    } else {
      // Mock data
      revenueData = {
        totalRevenue: 12500,
...
app.get("/api/vendor/earnings", firebaseAuthMiddleware, requireRole(["vendor", "admin"]), async (req, res) => {
  try {
    console.log(`💰 Vendor earnings requested by: ${req.mongoUser.email}`);
    
    // For now, return same as revenue
    const revenueResponse = await requireRole(["vendor", "admin"])(req, res, () => {});
    if (!res.headersSent) {
  ...
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      if (status === "approved") {
        ...
        // Find user by email first
        const userToUpdate = await User.findOne({
          $or: [{ uid: application.userId }, { email: application.userEmail }],
        });
  ...
  try {
    res.json({
      ...
      data: {
        ...
        isAdmin: req.mongoUser.role === "admin",
        adminEmailCheck: req.mongoUser.email === "mahdiashan9@gmail.com" || req.mongoUser.email === "islameshan451@gmail.com",
      },
    });
  } catch (error) {
    res.status(500).json({
...
app.post("/api/debug/approve-all", async (req, res) => {
  try {
    if (mongoose.connection.readyState === 1) {
      const result = await Ticket.updateMany({}, { verified: "approved", isActive: true });
      res.json({
        success: true,
        message: `Approved ${result.modifiedCount} tickets`,
        data: result,
...
// FIXED: Suspend user
app.put("/api/admin/users/:userId/suspend", firebaseAuthMiddleware, requireRole(["admin"]), async (req, res) => {
  try {
...
// FIXED: Activate user
app.put("/api/admin/users/:userId/activate", firebaseAuthMiddleware, requireRole(["admin"]), async (req, res) => {
  try {
    const { userId } = req.params;
...
app.get("/api/admin/bookings/:bookingId", firebaseAuthMiddleware, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      if (status === "cancelled" && updatedBooking.ticketId) {
        ...
        if (previousBooking && previousBooking.status === "confirmed") {
          const ticket = await Ticket.findById(updatedBooking.ticketId);
          if (ticket) {
            ticket.availableQuantity = Math.min(ticket.quantity, ticket.availableQuantity + previousBooking.quantity);
            await ticket.save();
            console.log(`✅ Restored quantity for ticket ${ticket._id}`);
          }
        }
...
app.get("/api/admin/reports", firebaseAuthMiddleware, requireRole(["admin"]), async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        reports: [
          ...
          { id: 3, name: "Vendor Performance", type: "vendors", period: "2024-01" },
        ],
      },
    });
  } catch (error) {
...
app.post("/api/payments/:paymentId/verify", firebaseAuthMiddleware, async (req, res) => {
  try {
...
app.get("/api/payments/:paymentId/status", firebaseAuthMiddleware, async (req, res) => {
  try {
    const { paymentId } = req.params;
...
app.post("/api/bookings/:bookingId/pay", firebaseAuthMiddleware, async (req, res) => {
  try {
    const { bookingId } = req.params;
...
app.get("/api/bookings/:bookingId/download", firebaseAuthMiddleware, async (req, res) => {
  try {
    const { bookingId } = req.params;
...
if (require.main === module) {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`👑 Admin Emails: mahdiashan9@gmail.com, islameshan451@gmail.com`);
    console.log(`🌐 CORS Enabled for: https://ticketbari-projects.web.app, http://localhost:5173`);
    console.log("=".repeat(70));
    ...
    console.log("  GET  /api/vendor/cards - Vendor cards (requires vendor/admin)");
    console.log("  GET  /api/admin/dashboard - Admin dashboard (requires admin)");
    console.log("=".repeat(70));
  });
}